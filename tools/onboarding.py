            ...
            try:
                with get_local_connection() as conn:
                    ...
                    if cursor.fetchone():
                        ...
                        full_address = f"{address} || [EMPLOYMENT]: {emp_string} || Alt Phone: {alt_phone} || Marital: {marital} || Dependants: {dependants}"
                        full_notes = f"{general_notes} || Bank: {bank_name} ({account_no}) || Supervisor: {emp1_supervisor} || Start Date: {emp1_date}"

                        cursor.execute("""
                            INSERT INTO clients (
                                first_name, last_name, id_number, phone, email, address, total_gross, salary, employer, work_days, pay_day, bank_name, account_no, status
                            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active')
                        """, (
                            first_name, last_name, id_number, phone, email, full_address, total_gross, net_income, emp1_name, ", ".join(work_days), pay_day, bank_name, account_no
                        ))
                        conn.commit()
                        st.balloons()
